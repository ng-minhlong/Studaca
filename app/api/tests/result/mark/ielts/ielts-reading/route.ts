...
// Convert raw score to IELTS Band Score (scale 9.0) - Reading Academic
function rawToIeltsReadingBandScore(correct: number): number {
...
export async function POST(request: Request) {
  try {
    ...
    const testType = type || "ielts-reading";
    ...
    if (!isValidType(testType) || testType !== "ielts-reading") {
      return NextResponse.json({ error: "Invalid test type for IELTS Reading" }, { status: 400 });
    }
    ...
    const bandScore = rawToIeltsReadingBandScore(correctCount);
    ...
    return NextResponse.json({
      ...
      idTest,
      type: testType,
      message: "IELTS Reading test marked successfully",
    });
  } catch (error: any) {
    console.error("Error in IELTS Reading marking API:", error);
    return NextResponse.json(
